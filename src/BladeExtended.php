...
class BladeExtended
{
	...
	protected function parseClass(&$finded)
	{
		...
		{
			$class = '{{ \Goszowski\BladeExtended\Helper::renderClass(' . $value . ') }}';
			$this->insertContent($finded['opening']['start'] + $matches['class'][1], $class);
			$finded['opening']['end'] += strlen($class);
		} else
		{
			$class = '{{ \Goszowski\BladeExtended\Helper::renderAttribute("class", ' . $value . ') }}';
	...
	protected function parseAttr(&$finded)
	{
		...
		$attr = '{{ \Goszowski\BladeExtended\Helper::renderAttribute("' . $attribute . '",' . $value . ') }}';
